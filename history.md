...
**JSON 工具页面布局重构 - 左右分栏设计**

- 重构 JsonPage.vue 页面布局
  - 从 Tabs 标签页布局改为左右分栏 Grid 布局
  - 左侧：代码编辑器 + 顶部工具栏 + 底部快捷操作栏
  - 右侧：树形视图 + 操作按钮 + 底部信息栏
  - 匹配 BeJSON 布局风格，提升用户体验
- 优化顶部工具栏功能
  - 格式化、压缩、转义（下拉菜单）
  - Unicode 转换（下拉菜单：中文转 Unicode、Unicode 转中文）
  - 中文逗号处理、代码折叠选项
- 优化底部快捷操作栏
  - 左侧：清空、示例
  - 中间：添加转义、去除转义、转 Get 参数
  - 右侧：Dict 转 Json、Json 转 Dict、旧版
- 优化右侧树形视图工具栏
  - 树形视图、代码转换、全展开、全折叠按钮
  - 底部信息栏显示格式信息（2 空格、14px、访问正版）
- 添加完整的 SCSS 样式
  - 使用 CSS 变量实现主题切换
  - 响应式布局（1200px、768px 断点）
  - 玻璃态设计风格，与项目整体风格一致
- 保留功能占位按钮（后续迭代实现）
  - 添加转义、去除转义、Dict 转 Json、Json 转 Dict、代码转换等
