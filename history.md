...
- 问题分析
  - 原代码使用 typeLabels['system_function']（值为"系统函数"）作为前缀
  - 新字段的 dataSource 默认值也是 'system_function'
  - 导致生成的字段名以"系统函数"开头，如"系统函数\_mkvycbnw"
- 修改方案
  - 移除未使用的 typeLabels 对象
  - 将默认前缀改为更通用的"custom_field"
  - 简化代码，避免中文前缀导致的命名不规范问题
- 修改效果
  - 新添加的字段默认名称为"custom_field_xxx"格式
  - 用户可以在添加后手动修改为有意义的字段名
  - 符合业务规范，更易于理解和维护
- 所有代码通过ESLint检查
