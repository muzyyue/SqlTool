...
const SQL_KEYWORD_PATTERN =
  /\b(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|TRUNCATE|BEGIN|COMMIT|ROLLBACK|GRANT|REVOKE|WITH|MERGE|CALL|DECLARE|EXEC|EXECUTE)\b/i;

/**
 * SQL语句起始正则（用于快速定位SQL片段）
 */
const SQL_START_PATTERN =
  /\b(?:SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM|CREATE\s+(?:TABLE|INDEX|VIEW|PROCEDURE|FUNCTION|TRIGGER|DATABASE|SCHEMA)|ALTER\s+(?:TABLE|INDEX|VIEW|PROCEDURE|FUNCTION|TRIGGER|DATABASE)|DROP\s+(?:TABLE|INDEX|VIEW|PROCEDURE|FUNCTION|TRIGGER|DATABASE)|TRUNCATE\s+TABLE|BEGIN|(?:DECLARE\s*.*?\s*)?BEGIN|DECLARE\b|WITH\s+\w+\s+AS\s*\(|MERGE\s+INTO|CALL\s+|EXEC(?:UTE)?\s+)\b/i;

/**
 * 日志/时间戳前缀正则（用于清理非SQL内容）
 */
const LOG_PREFIX_PATTERN =
  /^\s*(?:\[\d{4}[^\]]*\]|\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}(?:\.\d+)?(?:Z|[+-]\d{2}:?\d{2})?|\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2})\s*(?:[-:—–]\s*\w+\s*(?:[-:—–]\s*)?)?/gm;

/**
 * 代码块标记正则
 */
const CODE_BLOCK_PATTERN =
  /```(?:sql|SQL|mysql|postgresql|plsql|tsql)\s*\n([\s\S]*?)```/g;
...
export function extractSqlStatements(text, options = {}) {
  ...
  for (const rawStatement of statements) {
    const sqlText = trimWhitespace
      ? rawStatement.replace(/\s+/g, " ").trim()
      : rawStatement;
  ...
  if (!hasValidResult) {
    const inlineResults = extractInlineSql(text, {
      removeLogPrefix,
      ignoreComments,
      trimWhitespace,
    });
...
const INLINE_SQL_START =
  /\b(SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM|CREATE\s|ALTER\s|DROP\s|TRUNCATE|BEGIN|DECLARE\b|WITH\s+\w+\s+AS\s*\(|MERGE\s+INTO|CALL\s|EXEC(?:UTE)?\s)\b/i;

function extractInlineSql(
  text,
  { removeLogPrefix, ignoreComments, trimWhitespace },
) {
  let workingText = text;
...
function extractFromCodeBlocks(text) {
  const results = [];
  const regex =
    /```(?:sql|SQL|mysql|postgresql|plsql|tsql|oracle|mariadb)\s*\n([\s\S]*?)```/g;
  let match;
...
function extractTableNames(sql) {
  ...
  const joinPattern =
    /\b(?:INNER\s+|LEFT\s+(?:OUTER\s+)?|RIGHT\s+(?:OUTER\s+)?|FULL\s+(?:OUTER\s+)?|CROSS\s+|NATURAL\s+)?JOIN\s+([a-zA-Z_][a-zA-Z0-9_]*)/gi;
  // INTO 子句
...
function extractWhereConditions(sql) {
  ...
  const wherePattern =
    /\bWHERE\s+(.+?)(?:\s+GROUP\s+BY|\s+ORDER\s+BY|\s+LIMIT|\s+HAVING|\s*$)/is;
