      ...
      <div class="input-section">
        ...
        <div class="input-card" v-if="parsedFields.length > 0">
          <div class="card-header">
            <h3>条件字段配置</h3>
            <a-tooltip
              title="选择用于WHERE条件的字段，通常为主键或唯一标识字段"
            >
              <QuestionCircleOutlined />
            </a-tooltip>
          </div>
        ...
        <div class="input-card" v-if="parsedFields.length > 0">
          <div class="card-header">
            <h3>选择要修改的字段</h3>
            <a-tooltip
              title="选择需要在UPDATE语句中更新的字段，未选择的字段将不会被更新"
            >
              <QuestionCircleOutlined />
            </a-tooltip>
          </div>
          ...
          <div class="update-fields-config">
            ...
            <div v-if="updateFields.length > 0" class="update-fields-summary">
              <a-tag color="blue"
                >已选择 {{ updateFields.length }} 个字段</a-tag
              >
            </div>
          </div>
        </div>
...
const { operationLogs, getLogColor, formatTime, clearLogs, exportLogs } =
  useOperationLog();
...
watch(
  ...
  (
    [newConditionFields, newUpdateFields],
    [oldConditionFields, oldUpdateFields],
  ) => {
    // 只有当数据已加载时才处理
    ...
    const conditionChanged =
      !oldConditionFields ||
      JSON.stringify(newConditionFields.sort()) !==
        JSON.stringify(oldConditionFields.sort());
    const updateChanged =
      !oldUpdateFields ||
      JSON.stringify(newUpdateFields.sort()) !==
...
const clearMapping = (ddlFieldName) => {
  ...
  if (fieldInfo && fieldInfo.isCustom) {
    ...
    if (
      updateFieldIndex !== -1 &&
      !conditionFields.value.includes(ddlFieldName)
    ) {
...
const handleCustomBindingSave = (customFieldsData) => {
  ...
  try {
    ...
    logInfo(
      `已移除 ${originalLength - parsedFields.value.length} 个之前添加的自定义字段`,
    );
