...
export class ApiModelAdapter extends BaseModelAdapter {
  ...
  async initialize(): Promise<void> {
    if (!this.config.apiKey) {
      throw new Error(`API Key 未配置，请先配置 ${this.config.provider} 的 API Key`)
    }
   ...
   * 调用 OpenAI API
   */
  private async callOpenAI(prompt: string, options: GenerateOptions): Promise<ModelResponse> {
   ...
   * 调用 Anthropic API
   */
  private async callAnthropic(prompt: string, options: GenerateOptions): Promise<ModelResponse> {
   ...
   * 调用自定义 API
   */
  private async callCustomAPI(prompt: string, options: GenerateOptions): Promise<ModelResponse> {
 ...
 * 创建 OpenAI 适配器
 */
export function createOpenAIAdapter(config: Omit<ApiModelConfig, 'provider' | 'type'>): ApiModelAdapter {
 ...
 * 创建 Anthropic 适配器
 */
export function createAnthropicAdapter(config: Omit<ApiModelConfig, 'provider' | 'type'>): ApiModelAdapter {
  return new ApiModelAdapter({
    ...config,
    provider: 'anthropic',
    type: ModelType.API,
 ...
 * 创建自定义 API 适配器
 */
export function createCustomAPIAdapter(config: Omit<ApiModelConfig, 'type'>): ApiModelAdapter {
  return new ApiModelAdapter({
    ...config,
    type: ModelType.API,
  })
