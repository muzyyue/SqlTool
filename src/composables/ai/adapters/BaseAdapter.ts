...
export abstract class BaseModelAdapter implements IModelAdapter {
   ...
   * 生成嵌入向量（可选实现）
   */
  async generateEmbedding?(_text: string, _options?: EmbeddingOptions): Promise<EmbeddingResponse> {
   ...
   * 更新加载进度
   */
  protected updateProgress(loaded: number, total: number, status: string): void {
...
export function useAdapterState(adapter: BaseModelAdapter) {
  ...
  const onStateChange = (callback: (state: ModelState) => void) => {
    ...
    return () => adapter.removeEventListener('state-changed' as ModelEvent, listener)
  }
  ...
  const onLoadingProgress = (callback: (progress: LoadingProgress) => void) => {
    ...
    return () => adapter.removeEventListener('loading-progress' as ModelEvent, listener)
  }
