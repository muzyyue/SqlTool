...
export class LocalModelAdapter extends BaseModelAdapter {
  ...
  private async doInitialize(): Promise<void> {
    try {
      ...
      if (
        this.config.modelType === LocalModelType.TEXT_GENERATION ||
  ...
  private async loadTransformers(): Promise<TransformersModule> {
    ...
    try {
      ...
      console.log(
        `[AI] Transformers.js 加载完成，耗时: ${loadTime.toFixed(2)}ms`,
      );
      ...
      const loadTime = performance.now() - startTime;
      console.error(
        `[AI] 加载 Transformers.js 失败 (耗时: ${loadTime.toFixed(2)}ms):`,
        error,
      );
  ...
  private async loadTextPipeline(): Promise<TextGenerationPipeline> {
    ...
    const progressCallback = (progress: LoadingProgress) => {
      if (progress.status === "progress") {
        ...
        const overallProgress =
          20 + Math.round((progress.percentage / 100) * 70);
    ...
    try {
      ...
      const loadTime = performance.now() - startTime;
      console.log(
        `[AI] 文本生成模型加载完成，耗时: ${(loadTime / 1000).toFixed(2)}s`,
      );
      ...
      const loadTime = performance.now() - startTime;
      console.error(
        `[AI] 加载文本生成模型失败 (耗时: ${(loadTime / 1000).toFixed(2)}s):`,
        error,
      );
  ...
  private async loadEmbeddingPipeline(): Promise<EmbeddingPipeline> {
    ...
    try {
      ...
      const loadTime = performance.now() - startTime;
      console.log(
        `[AI] 嵌入模型加载完成，耗时: ${(loadTime / 1000).toFixed(2)}s`,
      );
      ...
      const loadTime = performance.now() - startTime;
      console.error(
        `[AI] 加载嵌入模型失败 (耗时: ${(loadTime / 1000).toFixed(2)}s):`,
        error,
      );
