...
export function useAiConfig() {
  ...
  const isAnyProviderConfigured = computed(() => {
    return Object.values(configState.value.providers).some(
      (p) => p.enabled && p.apiKey,
    );
  });

  const isLocalModelEnabled = computed(
    () => configState.value.localModel.enabled,
  );
  ...
  const getConfiguredProviders = (): ApiProvider[] => {
    return (Object.keys(configState.value.providers) as ApiProvider[]).filter(
      (p) =>
        configState.value.providers[p].enabled &&
        configState.value.providers[p].apiKey,
    );
  };
      ...
      if (imported.providers) {
        for (const [key, value] of Object.entries(imported.providers)) {
          if (configState.value.providers[key as ApiProvider]) {
            ...
            configState.value.providers[key as ApiProvider].enabled =
              providerConfig.enabled ?? false;
            if (providerConfig.model) {
              configState.value.providers[key as ApiProvider].model =
                providerConfig.model;
            }
            if (providerConfig.baseUrl) {
              configState.value.providers[key as ApiProvider].baseUrl =
