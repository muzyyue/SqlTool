...
export function useCustomBinding() {
  ...
  const removeCustomBinding = (ddlFieldName) => {
    const index = customBindings.value.findIndex(
      (binding) => binding.ddlFieldName === ddlFieldName,
    );
    if (index >= 0) {
  ...
  const validateBindings = () => {
    ...
    // 确保数据是数组
    const bindings = Array.isArray(customBindings.value)
      ? customBindings.value
      : [];
    const rules = Array.isArray(fieldConcatenationRules.value)
    ...
    bindings.forEach((binding) => {
      if (
        binding &&
        typeof binding.excelIndex === "number" &&
        binding.excelIndex < -1
      ) {
  ...
  const importBindings = (config) => {
    if (config.customBindings) {
      ...
      customBindings.value = Array.isArray(config.customBindings)
        ? config.customBindings
        : [];
    }
    if (config.fieldConcatenationRules) {
      // 确保fieldConcatenationRules始终是数组
      fieldConcatenationRules.value = Array.isArray(
    ...
    if (config.customFields) {
      const fields = Array.isArray(config.customFields)
        ? config.customFields
        : [];
      // 创建新数组触发响应式更新
  ...
  const removeCustomField = (fieldName) => {
    const index = customFields.value.findIndex(
      ...
      switch (field.dataSource) {
        case "system_function":
          ...
          result[field.fieldName] =
            `${field.systemFunctionConfig?.functionName || "NOW"}()`;
          break;
  ...
  const concatenationRuleCount = computed(
    () => fieldConcatenationRules.value.length,
  );
  const customFieldCount = computed(() => customFields.value.length);
  const totalCustomBindings = computed(
    () =>
      customBindingCount.value +
      concatenationRuleCount.value +
