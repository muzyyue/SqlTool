...
async function testDatabaseStrategies() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      console.log(`检测结果: ${detection.databaseType} (置信度: ${detection.confidence.toFixed(2)})`)
      
      // 测试DDL解析
      const parsedResult = await strategyManager.parseDdl(testCase.ddl, testCase.expectedType)
      console.log(`解析成功 - 表名: ${parsedResult.tableName}, 字段数: ${parsedResult.fields.length}`)
      
      // 显示字段信息
      parsedResult.fields.forEach((field, index) => {
        console.log(`  字段${index + 1}: ${field.name} ${field.type} ${field.nullable ? 'NULL' : 'NOT NULL'} ${field.isIdentity ? 'IDENTITY' : ''}`)
      })
      ...
      // 测试版本兼容性检查
      const versionCheck = strategyManager.checkVersionCompatibility(testCase.expectedType, 'latest')
