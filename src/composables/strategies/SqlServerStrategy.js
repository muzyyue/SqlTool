...
export class SqlServerStrategy extends DatabaseStrategy {
  constructor() {
    ...
    this.supportedVersions = ['2008', '2012', '2014', '2016', '2017', '2019', '2022']
  ...
  extractIndexes(ddlStatement) {
    ...
    // 提取索引定义
    const indexRegex = /CREATE\s+(?:UNIQUE\s+)?(?:NONCLUSTERED\s+)?INDEX\s+\[?([\w\]]+)\]?\s+ON\s+[\w\]]+\s*\(([^)]+)\)/gi
    let match
  ...
  extractConstraints(ddlStatement) {
    ...
    while ((fkMatch = foreignKeyRegex.exec(ddlStatement)) !== null) {
      constraints.push({
        ...
        referenceColumns: fkMatch[3].split(',').map(col => col.trim().replace(/[\[\]]/g, ''))
      })
    }
    
    // 提取默认值约束
    const defaultRegex = /CONSTRAINT\s+\[?([\w\]]+)\]?\s+DEFAULT\s+([^\s]+)\s+FOR\s+\[?([\w\]]+)\]?/gi
    let defaultMatch
