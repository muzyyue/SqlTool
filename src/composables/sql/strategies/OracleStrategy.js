...
export class OracleStrategy extends DatabaseStrategy {
  ...
  checkVersionCompatibility(version) {
    ...
    for (const supportedVersion of this.supportedVersions) {
      const supportedVersionNum =
        versionMap[supportedVersion] || parseInt(supportedVersion);
  ...
  extractTableName(ddlStatement) {
    const tableNameRegex =
      /CREATE\s+(?:GLOBAL\s+TEMPORARY\s+)?TABLE\s+(?:\w+\.)?([\w."]+)/i;
  ...
  parseFieldDefinition(fieldDef) {
    ...
    field.isIdentity =
      upperDef.includes("GENERATED") && upperDef.includes("AS IDENTITY");
