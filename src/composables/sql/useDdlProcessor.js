...
export function useDdlProcessor() {
    ...
    try {
      ...
      const finalResult = {
        ...
        details: {
          ...
          indexes:
            enhancedResult.indexes || typeParseResult.details.indexes || [],
          constraints:
            enhancedResult.constraints ||
            typeParseResult.details.constraints ||
            [],
  ...
  const convertDdl = async (ddlStatement, targetDatabase, options = {}) => {
    ...
    // 2. 生成目标数据库的DDL
    const generated = ddlGenerator.generateDdl(
      parseResult,
      targetDatabase,
      options,
    );
