...
export function useSqlGeneratorEnhanced() {
  ...
  const generateAutoIncrementValue = (
    ...
    if (
      customBindingManager &&
      customBindingManager.generateAutoIncrementValue
    ) {
      const groupValue = getGroupValue(
        row,
        customField.autoIncrementConfig?.groupBy,
        fieldMappings,
      );
      ...
      console.log(
        `警告: 自增字段 ${ddlField.name} 缺少customBindingManager，返回NULL`,
      );
      return "NULL";
    }
  };
  ...
  const generateExcelCombineValue = (customField, row, ddlField, dbType) => {
    ...
    if (formatTemplate) {
      combinedValue = formatTemplate.replace(
        /\{value(\d+)\}/g,
        (match, num) => {
   ...
   * @returns {string} 生成的字段值
   */
  const generateCustomFieldValue = (
    mapping,
    row,
    dbType,
    customBindingManager,
    ...
    // 如果仍然没有找到，尝试从 fieldConcatenationRules 中查找拼接规则
    if (
      !customField &&
      customBindingManager &&
      customBindingManager.fieldConcatenationRules
    ) {
      const concatenationRule =
        customBindingManager.fieldConcatenationRules.value.find(
          (rule) => rule.ddlFieldName === mapping.ddlField.name,
        );
      if (concatenationRule) {
        // 将拼接规则转换为自定义字段配置格式
        customField = {
          fieldName: concatenationRule.ddlFieldName,
          dataType: concatenationRule.dataType || "string",
          dataSource: "excel_combine",
          excelCombineConfig: {
            columns: concatenationRule.sourceColumns || [],
            separator: concatenationRule.separator || "",
            format: concatenationRule.format || "",
          },
        };
      }
    }
    ...
    if (customField) {
      ...
      switch (customField.dataSource) {
        case "system_function":
          return generateSystemFunctionValue(
            customField,
            dbType,
        ...
        case "excel_combine":
          return generateExcelCombineValue(
            customField,
            row,
            mapping.ddlField,
            dbType,
    ...
    if (funcInfo) {
      console.log(
        `字段 ${mapping.ddlField.name} 使用默认UUID函数: ${funcInfo.syntax}`,
      );
      return funcInfo.syntax;
    } else {
  ...
  const generateBatchInsertSql = (
    ...
    const originalOrder = new Map(
      fieldMappings.map((m, i) => [m.ddlField?.name, i]),
    );
    ...
    if (mappedFields.length === 0) {
      throw new Error(
        "没有有效的字段映射关系（所有字段都是自增主键、主键字段或未映射）",
      );
    }
    ...
    const fieldNames = mappedFields.map((mapping) => {
      ...
      if (!fieldName || String(fieldName).trim() === "") {
        throw new Error(
          `字段名不能为空（DDL字段: ${mapping.ddlField?.name || "unknown"}）`,
        );
      }
    ...
    // 验证字段名列表中没有空字符串
    const emptyFieldIndex = fieldNames.findIndex(
    ...
    batchData.forEach((row, rowIndex) => {
      const values = mappedFields.map((mapping) => {
        ...
        if (mapping.generatedByFunction === true) {
          ...
          return generateCustomFieldValue(
            mapping,
            row,
            dbType,
            customBindingManager,
        ...
        const hasValidExcelIndex =
          !isNaN(currentExcelIndex) && currentExcelIndex >= 0;

        // 检查是否是自定义字段且没有映射到Excel列
        if (
          mapping.ddlField.isCustom &&
          (!mapping.excelHeader || !hasValidExcelIndex)
        ) {
          console.log(`处理无映射的自定义字段: ${mapping.ddlField.name}`);
          return generateCustomFieldValue(
            mapping,
            row,
            dbType,
        ...
        if (!mapping.excelHeader || !hasValidExcelIndex) {
          console.log(
            `字段 ${mapping.ddlField.name} 未映射到Excel列，返回NULL`,
          );
          return "NULL";
        } else {
          console.log(
      ...
      console.log(
        `第${rowIndex + 1}行finalValues包含undefined:`,
        finalValues.includes(undefined),
      );
      ...
      console.log(
        `第${rowIndex + 1}行finalValues.join(', '):`,
        finalValues.join(", "),
      );
  ...
  const generateSingleUpdateSql = (
    ...
    fieldMappings.forEach((mapping) => {
      ...
      const isWhereField =
        whereFields && whereFields.includes(mapping.ddlField.name);
      if (
        !isWhereField &&
      ...
      if (mapping.generatedByFunction === true) {
        value = generateCustomFieldValue(
          mapping,
          row,
          dbType,
          customBindingManager,
    ...
    switch (dbType) {
      ...
      case "postgresql": {
        ...
        const needsQuotes =
          ...
          /^(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE|AND|OR|NOT|NULL|TRUE|FALSE)$/i.test(
            name,
          );
  ...
  const generateHeaderComment = (
    ...
    const filteredMappings = fieldMappings.filter(
      (mapping) => !mapping.ddlField.isIdentity,
    );
    ...
    filteredMappings.forEach((mapping) => {
      ...
      const excelInfo = mapping.excelHeader
        ? ` -> "${mapping.excelHeader}"`
        : "";
  ...
  const beautifySql = (sql, options = {}) => {
    ...
    // 2. 智能缩进和换行
    beautified = applySmartFormatting(
      beautified,
      indentSpaces,
      formatStyle,
      maxLineLength,
  ...
  const applySmartFormatting = (
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine.endsWith("(")) {
        ...
        const contentToProcess = startsWithComma
          ? trimmedLine.substring(1).trim()
          : trimmedLine;

        if (
          contentToProcess.length > maxLineLength &&
          formatStyle === "expanded"
        ) {
          const splitLine = splitLongLine(
            contentToProcess,
            maxLineLength,
          ...
          if (startsWithComma) {
            splitLine[0] =
              " ".repeat(currentIndent) + "," + splitLine[0].trim();
          }
  ...
  const alignValuesClauseSimple = (sql, indentSpaces) => {
    ...
    const valuesIndex = lines.findIndex((line) =>
      line.trim().toUpperCase().startsWith("VALUES"),
    );
      ...
      if (trimmedLine.endsWith(";")) {
        ...
        if (upperStmt.startsWith("INSERT")) {
          if (!upperStmt.includes("VALUES")) {
            errors.push(
              `第${statementCount}条INSERT语句（第${lineIndex + 1}行）缺少VALUES关键字`,
            );
          }
        }
        // 检查UPDATE语句格式
          ...
          if (!upperStmt.includes("SET")) {
            errors.push(
              `第${statementCount}条UPDATE语句（第${lineIndex + 1}行）缺少SET关键字`,
            );
          }
          if (!upperStmt.includes("WHERE")) {
            errors.push(
              `第${statementCount}条UPDATE语句（第${lineIndex + 1}行）缺少WHERE条件`,
            );
          }
        }
