...
export function useExcelDataProcessor() {
  ...
  const handleCellSplitToggle = (checked) => {
    ...
    if (!checked) {
      if (originalExcelDataForSplit.value.length > 0) {
        ...
        const restoredRowCount = excelData.value.length
        message.success(`单元格数据拆分已关闭，已恢复原始数据（${restoredRowCount} 行）`)
      } else {
      ...
      if (originalExcelDataForSplit.value.length === 0 && excelData.value.length > 0) {
  ...
  const applyCellSplit = () => {
    ...
    const separator =
      cellSplitSeparator.value === 'custom' ? customSeparator.value : cellSplitSeparator.value
    ...
    excelData.value.forEach((row) => {
      ...
      const rowSplitCount = Math.max(...columnSplits.map((splits) => splits.length))
    ...
    if (expandedRows > 0) {
      message.success(`拆分完成: 原始 ${originalRows} 行 → 拆分后 ${splitRows} 行`)
    } else {
      message.info('数据无需拆分')
    }
  }
