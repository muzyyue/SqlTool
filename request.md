...
12.
13.输出的 输入参数项  总是会转行，可能是复制的文本处理器中的数据就是转行的，测一测是否会转行，以及能否在 格式化（美化）过程中去掉转行       参数中的 \ 这个符号 在生成的sql中会变成  \\\   以及去除空格的功能（前置审查项、文件参数间的空格）excel中是没有空格的  应该是美化sql造成的  默认不需要美化sql（添加一个下载原始sql的按钮）
14.
15.
