...
10.
11.自定义字段-字段拼接中，选择目标DDL字段后无法添加到映射配置表中；输入ddl项会冲突 （缓存的原因 之前添加了后面删除掉这项，（缓存未删除）映射配置中未显示 添加新的字段项会弹出ddl项冲突 ）
12.自定义字段中字段拼接的格式化模板，输入框太小，无法显示所有文本
13.输出的 输入参数项  总是会转行，可能是复制的文本处理器中的数据就是转行的，测一测是否会转行，以及能否在 格式化（美化）过程中去掉转行       参数中的 \ 这个符号 在生成的sql中会变成  \\\   以及去除空格的功能（前置审查项、文件参数间的空格）excel中是没有空格的  应该是美化sql造成的  默认不需要美化sql
14.
15.
