...
7.添加默认配置 当出现created\_at 或者 updated\_at 时，自动使用 sysdate函数

8.update页面，添加自定义字段时，通过字段拼接添加的数据不会出现在映射列表中，而在自定义字段中添加的excel列组合的数据却可以  ； 点击生成sql后无反应   ； 不一定需要唯一字段

9. 

10.继续优化JSON部分的功能，首先优化ui 添加层级查找字段、转义、自动高亮错误位置的功能、扁平输出、添加、去掉转义符（nbsp \\n \\t 等等）11.自定义字段-字段拼接中，选择目标DDL字段后无法添加到映射配置表中；输入ddl项会冲突 （缓存的原因 之前添加了后面删除掉这项，（缓存未删除）映射配置中未显示 添加新的字段项会弹出ddl项冲突 ） 在字段拼接中，给字段配置添加一个数据类型以控制生成的sql中的数据类型（字符串、整数、小数等）12.在正常的自定义字段（字段拼接）与自定义字段使用自增数字的字段之间（数据库插入表中的之间）会缺少逗号13.输出的 输入参数项 总是会转行，可能是复制的文本处理器中的数据就是转行的，测一测是否会转行，以及能否在 格式化（美化）过程中去掉转行 参数中的 \\ 这个符号 在生成的sql中会变成 \\\\ 以及去除空格的功能（前置审查项、文件参数间的空格）excel中是没有空格的 应该是美化sql造成的 默认不需要美化sql（添加一个下载原始sql的按钮） 美化出问题了，添加一个复制原始sql的按钮14.重复操作性差，在已经生成了一遍sql的基础上，在进行修改（选择行范围）会导致原来的配置错误且不可使用，显示自定义绑定的表格无法显示所有添加的自定义字段，只会显示最新的添加的字段且编辑功能不可用（点击编辑会进入添加自定义字段弹窗，但没有数据显示） 在生成sql数据后，在进行行范围选择会导致之前编辑好的映射配置部分的数据被重置15.字段拼接中的勾选 “是否添加 '' ” 的选项消失了 ？ 现在添加函数有 '' 包裹着
...
19.excel填充工具，添加对行数据的填充 ，高级数据处理部分-源数据列、查询匹配度、**提取列**对中文名称无效果
