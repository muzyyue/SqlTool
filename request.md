...
8.抽屉打不开
9.
10.测试大数据量的表格（含合并单元格） 看生成的sql语句是否正确  那个检验鉴定的表
11.给启用数据去重功能添加一个重置功能（重置数据去重），现在的数据去重功能选择过后，即使取消启用数据去重，但去重的效果还存在
12
13.
14.在excel数据填充工具中（以获取整个表格的数据了），添加功能：选择一列数据，以这个数据列的每一个单元格中的数据为基础 ， 添加条件（分割符 逗号这类的）将分割的数据遍历，每个数据去目标列查询对应数据并获取行数据，选择想要获得的数据返回，最后将这一个单元格的数据进行拼接（自定义分割符）再返回，然后接着进行下一个单元格的数据，。。。
15.
