...
​	use Sequential Thinking  在生成的INSERT语句部分添加一个批量修改的功能和ui（下拉选择ddl字段，输入框输入数据），修改指定ddl字段/列的数据，将列数据为XX 的数据更改为 XX 
...
需优化：
1.当映射配置中有相同字段时，自定义添加字段添加弹窗警告，以及不能添加数据
2.可以添加数据库类型-达梦
4.数据去重部分，取消原来字段/换个字段去重时，会丢失原始数据，只会根据当前去重后的数据再次去重
5.批量修改添加字典功能，多数据批量修改  ????  给个key-value结构的json数据来进行列的批量修改？？？
...
**添加JSON模块，功能有：** 类似文本编辑器(像notepad++那样)？ Web版本的
