...
describe("jsonExtractor", () => {
  ...
  describe("extractByJsonPath", () => {
    ...
    it("应处理无效路径", () => {
      expect(
        extractByJsonPath({ name: "test" }, ["$.nonexistent"])[0].status,
      ).toBe("error");
    });
  });
  ...
  describe("extractAtomicValues - 智能解包核心功能", () => {
      ...
      const json = [
        {
          field: "files",
          type: "files",
          value: '[{"type": 1, "value": "询问笔录,起诉意见书"}]',
        },
      ];
      const results = extractAtomicValues(json);

      const targetValue = results.find(
        (r) => r.finalValue === "询问笔录,起诉意见书",
      );
    ...
    it("应生成完整的数据血缘链路", () => {
      ...
      expect(t.lineage.some((s) => s.action === "parse-json-string")).toBe(
        true,
      );
    });
    ...
    it("应在达到最大深度时停止并标记", () => {
      const json = {
        l1: JSON.stringify({
          l2: JSON.stringify({ l3: JSON.stringify({ l4: "deep" }) }),
        }),
      };
      expect(
        extractAtomicValues(json, { maxDepth: 2 }).filter(
          (r) => r.status === "depth-limit",
        ).length,
      ).toBeGreaterThan(0);
    });
    ...
    it("应检测循环引用", () => {
      ...
      json.self = json;
      expect(
        extractAtomicValues(json).filter((r) => r.status === "circular-ref")
          .length,
      ).toBeGreaterThan(0);
    });
    ...
    it("性能测试：1000个键的解包时间应小于500ms", () => {
      const obj = {};
      for (let i = 0; i < 1000; i++) obj[`k${i}`] = `v${i}`;
