...
test.describe("PostgreSQL INSERT 完整流程测试 - 多元化策略", () => {
  ...
  test("测试6: 切换到自定义字段标签页", async () => {
    ...
    const activeTab = await page.$(
      '.ant-tabs-tab-active:has-text("自定义字段")',
    );
  ...
  test("测试9: 选择 PostgreSQL 作为目标数据库", async () => {
    ...
    const selectedDb = await page.$(
      '.ant-radio-button-wrapper-checked:has-text("PostgreSQL")',
    );
  ...
  test("测试10: 完整流程 - 配置自定义字段并生成 SQL", async () => {
    ...
    const hasCurrentTimestamp = validateSystemFunction(
      sqlResult,
      "CURRENT_TIMESTAMP",
    );
    expect(hasCurrentTimestamp).toBe(true);
    reporter.recordResult(
      "步骤12: 验证 SQL 包含 CURRENT_TIMESTAMP 函数",
      true,
      "SQL 包含 CURRENT_TIMESTAMP 函数",
    );

    const noUuid = validateNoUuid(sqlResult);
    expect(noUuid).toBe(true);
    reporter.recordResult(
      "步骤13: 验证 SQL 不包含 UUID",
      true,
      "SQL 正确使用系统函数而非 UUID",
    );
  ...
  test("测试16: 兼容性测试 - 不同数据库类型", async () => {
    ...
    const selectedDb = await page.$(
      '.ant-radio-button-wrapper-checked:has-text("MySQL")',
    );
    expect(selectedDb).not.toBeNull();
 ...
 * 分支测试模块：拼接字段功能测试
 * ============================================================================
 * 本模块测试从 fieldConcatenationRules 数据源查找拼接配置的场景
 * 包括：UI 显示验证、SQL 生成验证、边界情况测试
 */
test.describe("分支测试：拼接字段功能测试", () => {
  let page;
  let reporter;
  ...
  /**
   * 测试20: 添加拼接字段并验证 UI 显示
   */
  test("测试20: 添加拼接字段并验证 UI 显示", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    const addSuccess = await addConcatenationField(page, concatConfig);
    expect(addSuccess).toBe(true);
    ...
    reporter.recordResult(
      "添加拼接字段并验证 UI 显示",
      true,
      "拼接字段成功显示在 CustomFieldManager 中",
    );
  });
  ...
  test("测试21: 验证拼接字段配置详情显示", async () => {
    ...
    const configValid = await validateFieldConfig(page, "file_info_combined", {
      dataSource: "Excel组合",
      columns: [0, 1],
    });
    expect(configValid).toBe(true);

    reporter.recordResult("验证拼接字段配置详情显示", true, "配置详情正确显示");
  });

  /**
   * 测试22: 生成包含拼接字段的 SQL
   */
  test("测试22: 生成包含拼接字段的 SQL", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    const addSuccess = await addConcatenationField(page, concatConfig);
    expect(addSuccess).toBe(true);
    reporter.recordResult("步骤4: 添加拼接字段配置", true);

    await saveCustomFieldConfig(page);
    reporter.recordResult("步骤5: 保存配置", true);

    await selectDatabaseType(page, "PostgreSQL");
    reporter.recordResult("步骤6: 选择数据库类型", true);

    await clickGenerateSql(page);
    await page.waitForTimeout(3000);
    reporter.recordResult("步骤7: 生成 SQL", true);

    const sqlResult = await getGeneratedSql(page);
    expect(sqlResult.hasInsert).toBe(true);
    expect(sqlResult.hasValues).toBe(true);

    const hasConcatField = sqlResult.fullText.includes("file_display_name");
    expect(hasConcatField).toBe(true);
  ...
  test("测试23: 多个拼接字段同时配置", async () => {
    ...
    await addConcatenationField(page, concatConfig1);
    ...
    await addConcatenationField(page, concatConfig2);
  ...
  /**
   * 测试24: 拼接字段与系统函数字段混合配置
   */
  test("测试24: 拼接字段与系统函数字段混合配置", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    await addCustomField(page, systemFieldConfig);
    ...
    const hasSystemFunction = validateSystemFunction(
      sqlResult,
      "CURRENT_TIMESTAMP",
    );
    expect(hasSystemFunction).toBe(true);

    const hasConcatField = sqlResult.fullText.includes("file_full_info");
    expect(hasConcatField).toBe(true);
  ...
  test("测试25: 格式化模板拼接字段", async () => {
    ...
    const concatConfig = {
      fieldName: "formatted_file_info",
      dataType: "varchar",
      sourceColumns: [1, 2],
      separator: "",
      format: "文件名:{value1}, 路径:{value2}",
    };
    await addConcatenationField(page, concatConfig);
    await saveCustomFieldConfig(page);
    ...
    reporter.recordResult("格式化模板拼接字段", true, "格式化模板正确应用");
  });

  /**
   * 测试26: 边界测试 - 空源列配置
   */
  test("测试26: 边界测试 - 空源列配置", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    reporter.recordResult("边界测试 - 空源列配置", true, "正确处理空源列配置");
  });

  /**
   * 测试27: 边界测试 - 单列拼接
   */
  test("测试27: 边界测试 - 单列拼接", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    const singleColumnConfig = {
      fieldName: "single_column_field",
      dataType: "varchar",
      sourceColumns: [1],
      separator: "",
    };
    const addSuccess = await addConcatenationField(page, singleColumnConfig);
    expect(addSuccess).toBe(true);
    ...
    reporter.recordResult("边界测试 - 单列拼接", true, "单列拼接正确处理");
  });

  /**
   * 测试28: 完整流程 - 拼接字段端到端测试
   */
  test("测试28: 完整流程 - 拼接字段端到端测试", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    const hasConcatField = sqlResult.fullText.includes("file_complete_info");
    expect(hasConcatField).toBe(true);
   ...
   * 测试29: 验证拼接字段在重置后正确清除
   */
  test("测试29: 验证拼接字段在重置后正确清除", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
  ...
  /**
   * 测试30: 验证拼接字段与 DDL 字段映射共存
   */
  test("测试30: 验证拼接字段与 DDL 字段映射共存", async () => {
    await page.goto("http://localhost:5173/SqlTool/#/insert");
    await waitForPageLoad(page);
    ...
    expect(sqlResult.fullText).toContain("file_id");
    expect(sqlResult.fullText).toContain("file_name");
    expect(sqlResult.fullText).toContain("file_path");
    expect(sqlResult.fullText).toContain("additional_info");
